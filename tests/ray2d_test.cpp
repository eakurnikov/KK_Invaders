...
TEST(ray2d_test, test_initializer_list)
{
  ...
  Ray2D r4 = { 0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f};
  EXPECT_EQ(r4.initial().x(), 2.0f);
  EXPECT_EQ(r4.initial().y(), 3.0f);
  EXPECT_EQ(r4.direction().x(), 0.0f);
  EXPECT_EQ(r4.direction().y(), 1.0f);
}
...
TEST(ray2d_test, test_calculus)
{
  ...
  Ray2D r1 = { { 4.0f, 0.0f }, { 1.0f, 1.0f } };
  EXPECT_EQ(-r1 , Ray2D(-4.0f, 0.0f, 1.0f, 1.0f));
...
{
  std::stringstream s;
  s << Ray2D( { 0.0f, 1.0f }, { -1.0f, -1.0f } );
  EXPECT_EQ(s.str(), "Ray2D {initial point: {-1, -1}, direction: {0, 1}}");
}
